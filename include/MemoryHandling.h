...
WifiCredentials getCredentials(){
    ...
    serialLog("get Wifi Credentials from EEPROM", LOGSTATE_LOG);
    EEPROM.begin(512);
...
bool setCredentials(WifiCredentials new_credentials){
    serialLog("set Wifi Credentials on EEPROM", LOGSTATE_LOG);
    uint addr = 0;
...
bool setRGBMem(RGB rgb){
    ...
    serialLog("save RGB values on EEPROM", LOGSTATE_LOG);
    uint addr = 1;
    EEPROM.put(addr, tmp);
    EEPROM.commit();
